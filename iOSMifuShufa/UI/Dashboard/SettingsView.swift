    ...
    VStack(spacing: 0) {
      ...
      ScrollView {
        VStack(spacing: 0) {
          ...
          HStack {
            let chs = """
  当集字找不到单字图片时，可对字进行部件拆解，获取含有本身和部件的单字作为参考和候补。如「楚」字，如无单字结果，搜索含有本身「楚」的单字作为候补，如「礎」等。
  """
            let cht = """
  當集字找不到單字圖片時，可對字進行部件拆解，獲取含有本身和部件的單字作爲參考和候補。如「楚」字，如無單字結果，搜索含有本身「楚」的單字作爲候補，如「礎」等。
  """
            Text(chs.orCht(cht))
            .font(.footnote).foregroundStyle(.gray)
            .multilineTextAlignment(.leading)
